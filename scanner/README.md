...
- **Every gate metric is net of transaction costs (2026-08-08).** `walk_triple_barrier` stores outcomes gross - it charges nothing - so before this change rank IC, average R, precision and the HAC t-stats all described an edge no execution could capture, and the only cost-aware number in the report (`average_return_pct_after_slippage`, a flat 5 bps) was cosmetic and read by no gate. `compute_edge_validation_report` now charges `EDGE_COST_BPS_PER_SIDE` (default 25 bps/side = 0.50 return pct pts round trip, the pre-registered basis from the 20260710 sprint's `e4_decision`) against every row BEFORE any metric is computed, re-divides by the stored `risk_pct_used` to keep net R on the R scale, and recomputes the win/loss label from the net return so precision is net too. Gross values stay visible as `gross_*` / `average_gross_return_pct` for diagnosis, and the report stamps a `cost_model` block. The audit's `costs_charged` check fails closed unless the block has a positive cost, the exact implied round-trip charge, `net_of_costs` basis, and coverage of returns, R multiples, and win/loss labels; a decorative or partially migrated cost stamp cannot open a gate. Charging costs moved bullish from "unproven, +0.05 avg daily R" to blocked at -0.06, and the top decile from -0.02 to -0.13 avg R.
