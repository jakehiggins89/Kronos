...
- Live alerting also requires a current Edge Readiness Audit with `readiness=paper_trade_only`; run `--mode run_edge_lab` first. Preflight verifies the scan and validation timestamps inside the audit are both under 24 hours old and carry the same evidence-lab run ID, so rewriting `audit_edge` from stale artifacts cannot refresh the live gate. Immediately before any Telegram send, the exact current ticker/direction is rescored and must itself earn an Edge `promote` recommendation in a validation-supported direction; a different promoted ticker cannot authorize the alert.
